...
++  agent
  ...
  ++  on-poke
    ...
    ::  Load this back
      ...
      =^  cards  agent
        %-  rundef:w
        ;<  =_ag  bind:w  (write:w (on-load:ag [-:on-save:ag state.old]))
        :: ;<  =_ag  bind:w
        ::   %+  wurn  (diff ~(tap by wex.old) ~(tap by wex.dish))
        ::
        ::
        ::   %^  roll-write  (bind:w _ag)  (diff ~(tap by wex.old) ~(tap by wex.dish))
        ::   |:  [*[[=wire =ship *] *] ag=ag]
        ::   (write:w (on-agent:ag(src.+< ship) wire %kick ~))
      :: ::   ::;<  ~  _discard  !!
